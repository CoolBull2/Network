...
def get_network_health(host="google.com", packets=4):
    ...
    try:
        ...
        if packet_loss == 100:
            return {"status": "critical", "reason": "No response from host.","latency": f"{avg_latency}","packetloss":f"{packet_loss}"}
        elif packet_loss > 50 or avg_latency > 500:
            return {"status": "critical", "reason": f"High latency: {avg_latency} ms, {packet_loss}% packet loss.","latency": f"{avg_latency}","packetloss":f"{packet_loss}"}
        elif packet_loss > 10 or avg_latency > 200:
            return {"status": "warning", "reason": f"Moderate latency: {avg_latency} ms, {packet_loss}% packet loss.","latency": f"{avg_latency}","packetloss":f"{packet_loss}"}
        else:
            return {"status": "healthy", "reason": f"Stable connection: {avg_latency} ms, {packet_loss}% packet loss.","latency": f"{avg_latency}","packetloss":f"{packet_loss}"}
